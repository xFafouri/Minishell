...
/*   Updated: 2024/09/02 11:22:18 by sbourziq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (av == NULL)
		(ft_lstclear(gc), ft_lstclear(&g_lb->addres_fd), exit(0));
	if (av[0] == '.' && av[1] == '\0')
	{
	...
	if (av[0] == '\0')
	{
		...
		(ft_lstclear(gc), ft_lstclear(&g_lb->addres_fd), exit(127));
	}
}
...
char	**ft_helper_find_path(t_node **gc, t_cmd *ptr)
{
	...
	if (ptr->flag == 1)
	{
		commond_path = ft_split("/nfs/homes/sbourziq/bin:\
		/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:\
		/sbin:/bin:/usr/games:/usr/local/games:/snap/bin", ':', gc);
		return (commond_path);
	}
	while (ptr->env[i])
	{
		...
		if (access((commond_path)[i], F_OK) == 0)
		{
			if (access((commond_path)[i], X_OK) == 0)
				*path = (commond_path)[i];
				...
				(ft_lstclear(gc), ft_lstclear(&ptr->addres_fd), exit(127));
		}
		i++;
	}
	return (1);
